...
class MainWidget(QtGui.QWidget):
    ...
    def appendEvent(self, event):
        self.eventList.appendEvent(event)

    def signalAlarm(self, sender):
        # for i in self.CA.getZones():
        #     if i.getAlarm(): self.appendEvent(Event("Alarm włamaniowy z strefy "+i.getName()))
        self.appendEvent(Event(alarmType=EventType.BreakIn, zone=sender.getName()))
...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        for integraSystem in integraSystems:
            ...
            CA.hasZonesAlarmChanged.connect(self.mainWidget.signalAlarm)

            port = 7091
            if integraSystem.port:
                port = int(integraSystem.port)
            ...
            self.allParsers.append(CAParser)
            CAParser.start()
    ...
    def showLegend(self):
        self.legendCA=integra.Integra128()
